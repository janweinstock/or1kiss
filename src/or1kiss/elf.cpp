...
elf_symbol::elf_symbol(const char* name, Elf32_Sym* symbol):
    ...
    switch (type) {
    case STT_OBJECT:
        m_type = ELF_SYM_OBJECT;
        break;
    case STT_FUNC:
        m_type = ELF_SYM_FUNCTION;
        break;
    default:
        m_type = ELF_SYM_UNKNOWN;
        break;
    }
}
...
elf_section::elf_section(Elf* elf, Elf_Scn* scn):
    ...
    if (elf_getshdrstrndx(elf, &shstrndx) != 0)
        OR1KISS_ERROR("Call to elf_getshdrstrndx failed (%s)", elf_errmsg(-1));

    char* name = elf_strptr(elf, shstrndx, shdr->sh_name);
    if (name == NULL)
        OR1KISS_ERROR("Call to elfstrptr failed (%s)", elf_errmsg(-1));
    ...
    for (unsigned int i = 0; i < ehdr->e_phnum; i++) {
        ...
        if ((start != 0) && (shdr->sh_offset >= start) &&
            (shdr->sh_offset < end))
            m_phys_addr = phdr[i].p_paddr + (shdr->sh_addr - phdr[i].p_vaddr);
    }
...
void elf_section::load(env* e, bool verbose) {
    ...
    if (e->convert_and_transact(req) != RESP_SUCCESS) {
        fprintf(stderr,
                "warning: cannot load section '%s' to memory [0x%08" PRIx64
                " - 0x%08" PRIx64 "]",
...
elf::elf(const string& fname):
    m_filename(fname), m_endianess(), m_entry(), m_sections(), m_symbols() {
    ...
    case ELFDATA2LSB:
        m_endianess = ENDIAN_LITTLE;
        break;
    case ELFDATA2MSB:
        m_endianess = ENDIAN_BIG;
        break;
    default:
    ...
    while ((scn = elf_nextscn(e, scn)) != 0) {
        ...
        if (shdr->sh_type == SHT_SYMTAB) {
            ...
            for (unsigned int i = 0; i < num_symbols; i++) {
                char* name = elf_strptr(e, shdr->sh_link, symbols[i].st_name);
                if (name == NULL)
                    OR1KISS_ERROR(elf_errmsg(-1));
