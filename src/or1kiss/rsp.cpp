...
rsp::rsp(): m_trace(false), m_socket(-1), m_connection(-1), m_addr() {
...
rsp::rsp(unsigned short port):
    m_trace(false), m_socket(-1), m_connection(-1), m_addr() {
    char* env = getenv("OR1KISS_TRACE_RSP");
    if ((env != 0) && (*env != '0')) {
        m_trace = true;
        std::cout << "(or1kiss::rsp) tracing enabled" << std::endl;
...
void rsp::open(unsigned short port) {
    ...
    // Show output
    std::cout << "or1kiss: listening on port " << port << "... " << std::flush;
...
char rsp::recv_char() {
    ...
    case -1:
        OR1KISS_ERROR("Error receiving data (%d)", strerror(errno));
    case 0:
        close();
        return 0;
    default:
...
void rsp::send_char(char c) {
    ...
    case -1:
        OR1KISS_ERROR("Error sending data (%s)", strerror(errno));
    case 0:
        close();
        break;
    default:
