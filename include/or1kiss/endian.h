...
namespace or1kiss {
    ...
    template <typename T> inline T byte_swap(const T& );

    template <> inline u16 byte_swap<u16>(const u16& x) {
        return __bswap_16(x);
    }

    template <> inline u32 byte_swap<u32>(const u32& x) {
        return __bswap_32(x);
    }

    template <> inline u64 byte_swap<u64>(const u64& x) {
        return __bswap_64(x);
    }
    ...
    inline void* memcpyswp(void* to, void* from, unsigned int size) {
        ...
        case 4: *(u32*)to = byte_swap(*(u32*)from); break;
        case 8: *(u64*)to = byte_swap(*(u64*)from); break;
        default: {
                u32* src = (u32*)from;
                u32* dst = (u32*)to;
                while (size > 3) {
