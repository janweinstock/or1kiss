...
// OR1KISS_REG_VERSION initializes the Version register
#define OR1KISS_REG_VERSION                                    \
    ((OR1KISS_VER & 0xff) << 24 | (OR1KISS_CFG & 0xff) << 16 | \
     (OR1KISS_UVRP & 0x01) << 6 | (OR1KISS_REF & 0x3f))
...
#define OR1KISS_CPU_VERSION \
    ((OR1KISS_CPU_ID & 0xff) << 24 | (OR1KISS_CPU_VER & 0xffffff))
...
#define OR1KISS_ARCH_VERSION                                                 \
    ((OR1KISS_ARCH_MAJOR & 0xff) << 24 | (OR1KISS_ARCH_MINOR & 0xff) << 16 | \
...
#define OR1KISS_EX_TLB_MISS(imem) ((imem) ? EX_ITLB_MISS : EX_DTLB_MISS)
...
typedef struct watchpoint {
    ...
    bool overlaps(const struct watchpoint& wp) const;
} watchpoint;
...
typedef struct watchpoint_event {
    u32 addr;
    u32 size;
    u64 wval;
...
private:
    ...
    void setup_fp_round_mode();
    void reset_fp_round_mode();

    void reset_fp_flags(float result);
    void reset_fp_flags(double result);

    bool breaks_quantum(const instruction* insn);

    void schedule_jump(u32 target, u32 delay);
...
public:
    ...
    void set_insn_ptr(unsigned char* ptr, u32 addr_start = 0x00000000,
                      u32 addr_end = 0xffffffff, u64 cycles = 0);
    void set_data_ptr(unsigned char* ptr, u32 addr_start = 0x00000000,
                      u32 addr_end = 0xffffffff, u64 cycles = 0);
...
inline bool or1k::is_sleeping() const {
    return is_sleep_allowed() && !is_exception_pending() && (m_pmr & PMR_DME);
}
...
inline void or1k::setup_fp_round_mode() {
    ...
    case FPS_RMD:
        ...
        break;
    default:
        OR1KISS_ERROR("Unknown rounding mode (%d)", rounding_mode);
    }
}
